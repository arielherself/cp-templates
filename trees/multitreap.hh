...
template <typename T> class multitreap {
...
private:
    ...
    using element_type = pair<value_type, size_type>;
    ...
    void split(size_type u, value_type val, size_type &x, size_type &y) {
        if (!u) x = y = 0;
            ...
            if (!greater<element_type>()(fhq[u].val, {val, cnt})) x = u, split(rson, val, rson, y);
    ...
    value_type askNum(size_type u, size_type rank) {
        if (fhq[lson].size + 1 == rank) return fhq[u].val.first;
        if (fhq[lson].size >= rank) return askNum(lson, rank);
        else return askNum(rson, rank - fhq[lson].size - 1);
    }

public:
    multitreap() : fhq(1), cnt(0), root(0), _size(0) {}
    explicit multitreap(size_type n) : fhq(1), cnt(0), root(0), _size(0) {
        fhq.reserve(n + 1);
    }

    template<typename _InputIterator,
             typename = std::_RequireInputIter<_InputIterator>>
        multitreap(_InputIterator __first, _InputIterator __last)
